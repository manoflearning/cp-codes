...
int N;
string A[MAXN];
short C[MAXN][MAXN] __attribute__ ((aligned (32)));
    ...
    for (int i = 0; i < N; i++) {
        for (int j = i + 1; j < N; j++) {
            ...
            for (int k = j + 1; k < N; k += 16) {
                if (k + 16 <= N) {
                    __m256i Cik = _mm256_load_si256((const __m256i*)&C[i][k]);
                    __m256i Cjk = _mm256_load_si256((const __m256i*)&C[j][k]);
