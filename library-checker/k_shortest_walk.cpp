...
vector<ll> kth_walk(int s, int e, int K) {
  vector<int> nxt(n + 1); vector<int> top; vector<bool> vst(n + 1);
  vector<ll> dst(n + 1, INF); dst[e] = 0;
  ...
  while(!Q.empty()) {
    ...
    vst[x] = 1; nxt[x] = p;
    top.push_back(x);
    for(auto [c, y] : rgph[x]) if(!vst[y] && dst[y] > d + c) dst[y] = d + c, Q.push({d + c, y, x});
  }
  ...
  for(auto x : top) if(dst[x] < INF) {
    ...
    for(auto [c, y] : gph[x]) if(dst[y] < INF) {
      if(!chc[x] && y == nxt[x] && dst[x] == c + dst[y]) { chc[x] = 1; continue; }
...
int main() {
    ...
    auto ans = kth_walk(st, en, k);
    for (int i = 0; i < k; i++) {
        cout << (sz(ans) <= i ? -1 : ans[i]) << '\n';
    }
}
