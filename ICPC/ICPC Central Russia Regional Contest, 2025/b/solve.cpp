...
bigint get_bound(const bigint &x) {
    const bigint cnt_o = (x + 2) / 2;
    const bigint cnt_e = (x + 1) / 2;
    
    return (cnt_o * cnt_o) * 30 + (cnt_e * cnt_e + cnt_e) * 20;
}
...
int main() {
    ...
    bigint l(0), r(x);
    while (l < r) {
        const bigint mid = (l + r) / 2;

        if (x < get_bound(mid)) r = mid;
        else l = mid + 1;
    }

    // DEBUG
    // for (bigint l = 0; l <= 10; l += 1) {
    //     cout << l << ' ' << get_bound(l) << '\n';
    // }

    bigint score = get_bound(l - 1);
    while (1) {
        if (x <= score) break;
        score += (l + 1) * 10;
    }

    if (score == get_bound(l)) l += 1;
