...
void sol() {
    ...
    for(int i=0; i<m; i++) {
        if(arr[0][i]==0) {
            int now=1;
            if(i-1>=0 && arr[0][i-1]==now) now++;
            if(i+1<n && arr[0][i+1]==now) now++;
        ...
        if(arr[n-1][i]==0) {
            int now=1;
            if(i-1>=0 && arr[n-1][i-1]==now) now++;
            if(i+1<n && arr[n-1][i+1]==now) now++;
    ...
    for(int i=0; i<n; i++) {
        if(arr[0][i]==0) {
            int now=1;
            if(i-1>=0 && arr[i-1][0]==now) now++;
            if(i+1<m && arr[i+1][0]==now) now++;
        ...
        if(arr[n-1][i]==0) {
            ...
            if(i-1>=0 && arr[i-1][m-1]==now) now++;
            if(i+1<m && arr[i+1][m-1]==now) now++;
    ...
    while(mn<nn) {
        nn--;
        for(int i=1; i<m-1; i++) {
            arr[nn][i]=get_val2(nn, i);
        }
    }
    while(mn<mm) {
        mm--;
        for(int i=1; i<n-1; i++) {
            arr[i][mm]=get_val2(i, mm);
        }
    }
